...
for idx_set = 1:length(dataset)
    ...
    for idx_im = 1:length(filepaths)
        ...
        for scale = scale_all
            ...
            if strcmp(degradation, 'BI')
                im_HR_size = size(im_HR);
                hei = im_HR_size(1);
                wid = im_HR_size(2);
                im_LR = imresize(im_HR, [hei/scale, wid/scale], 'bicubic');
                im_LRHR = imresize(im_LR, [hei, wid], 'bicubic');
            ...
            folder_LRHR = fullfile(['./LR/HR', degradation], dataset{idx_set}, ['x', num2str(scale)]);
            if ~exist(folder_HR)
                mkdir(folder_HR)
            end
            if ~exist(folder_LR)
            ...
            if ~exist(folder_LRHR)
                mkdir(folder_LRHR)
            end
            % fn
            fn_HR = fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], [name_im(1:end-4), '.png']);
            fn_LR = fullfile(['./LR/LR', degradation], dataset{idx_set}, ['x', num2str(scale)], [name_im(1:end-4), '.png']);
            fn_LRHR = fullfile(['./LR/HR', degradation], dataset{idx_set}, ['x', num2str(scale)], [name_im(1:end-4), '.png']);
            imwrite(im_HR, fn_HR, 'png');
            imwrite(im_LR, fn_LR, 'png');
            imwrite(im_LRHR, fn_LRHR, 'png');
        end
        fprintf('\n');
    end
    fprintf('\n');
